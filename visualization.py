...
def create_visualization(data_csv='india_solar_data.csv', geojson_path='india-soi.geojson',
                         output_html='india_solar_potential.html', output_image='solar_potential_high_res.png'):
    ...
    # Masking grid points outside India's boundaries
    india_union = india.unary_union
    grid_points = np.array([Point(lon, lat) for lon, lat in zip(grid_lon.flatten(), grid_lat.flatten())])
    ...
    colormap = cm.LinearColormap(
        ...
        vmin=np.nanmin(values), vmax=np.nanmax(values), caption='Solar Potential (kWh/m²/year)'
    )

    # Matplotlib for high-res static image
    fig, ax = plt.subplots(figsize=(20, 24))
    ax.set_axis_off()
    img = ax.imshow(grid_z, cmap=ListedColormap(colormap.colors), extent=[bounds[0], bounds[2], bounds[1], bounds[3]],
    ...
    # Folium map with image overlay
    logger.info("Creating Folium map with overlay")
    m = folium.Map(location=[(bounds[1] + bounds[3]) / 2, (bounds[0] + bounds[2]) / 2], zoom_start=5, tiles='CartoDB positron')
    folium.raster_layers.ImageOverlay(image=output_image, bounds=[[bounds[1], bounds[0]], [bounds[3], bounds[2]]],
                                      opacity=0.7, interactive=True, cross_origin=False, zindex=1).add_to(m)
    
    # Adding India's boundary layer
    folium.GeoJson(india, style_function=lambda x: {'color': 'black', 'weight': 1.5, 'fillOpacity': 0}).add_to(m)
    colormap.add_to(m)
